...
/* Début du propre de la page : Grand-Livre */



//risque d'injection sur cette page sur toute les variable d'entrée
include('include/include.php');

//initialisation des paramêtre si il ne sont pas issu du formulaire
$total_debit=0;
$total_credit=0;

// le critère de tri > MARCHE PAS LE TRI SUR LES DTE
if (isset($_POST['tri']))
{
if($_POST['tri']=='doc_date')
$tri='doc_date';
}
else
{
...
//le numero de compte début
if (isset($_POST['compte_debut']))
{
$compte_debut=(int)$_POST['compte_debut'];
}
else
{
$compte_debut='0';
...
//le numero compte fin
if(isset($_POST['compte_fin']))
{
$compte_fin=(int)$_POST['compte_fin'] ;
}
else
{
$compte_fin='9';
...
echo '<center><form method="post" action="grandlivre.php"><p>';

$periode=$_SESSION['periode'] ;
$listecompte=lister_compte($bdd,$periode);

//liste deroulante debut
echo "Du Compte<select name=\"compte_debut\">";
foreach($listecompte as $compte)
{
  echo '<option value="' . $compte . '">' . $compte . ' - ' . label($bdd,$compte) . '</option>' ;
}
echo "</select>";

//liste deroulante fin

echo "Au Compte<select name=\"compte_fin\">";
foreach($listecompte as $compte)
{
  echo '<option value="' . $compte . '">' . $compte . ' - ' . label($bdd,$compte) . '</option>' ;
}
...
// bounton pour le type de tri  
// risque d'injection !!!!!
?>

<input type="radio" name="tri" value="numero_compte" id="compte" /> <label for="compte">Tri par numero de compte</label>
<input type="radio" name="tri" value="doc_date" id="date" /> <label for="date">Tri par date</label>
...
$listeecriture=recuperer_ecriture($bdd,$periode,array($compte_debut,$compte_fin),$tri); // on récupère les écriture
