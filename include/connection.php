...
$ecriture = $bdd->prepare('SELECT value FROM `'.MAIN_DB_PREFIX.'const` WHERE name = "MAIN_INFO_SOCIETE_NOM"');                    
...
$ecriture = $bdd->prepare('SELECT pcg_version FROM `'.MAIN_DB_PREFIX.'accounting_system` WHERE active = 1');                    
$ecriture->execute( array() );
$donnees = $ecriture->fetch();
...
$ecriture = $bdd->prepare('SELECT rowid,login ,pass_crypted FROM `'.MAIN_DB_PREFIX.'user` WHERE login = ?');                    
...
$ecriture = $bdd->prepare('SELECT fk_user,fk_id FROM `'.MAIN_DB_PREFIX.'user_rights` WHERE fk_user = ? AND fk_id=50411');                    
